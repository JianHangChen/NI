...
void gInitSquat(int StepNum); // 為了蹲站動作初始化
...
void gAKHS(void);	// WZ 140121 AnkleKneeHipStrategy
...
public:
	...
	CButton CheckManualMode;// 是否線上手動給軌跡模式(只有在實驗模式下)
	CButton CheckPE;// 按鈕控制項定義
	CButton CheckEmergentStop; // 按鈕控制項定義
	CButton CheckArmCtrl; // 是否讀Arm檔 (實驗模式開Thread)
	CButton CheckHandCtrl; // 是否讀Hand檔
public:
	...
	afx_msg void OnBnClickedButton6(); // 按鈕 "Send" 手動模式專用 13年隱藏!!!!!!!!
