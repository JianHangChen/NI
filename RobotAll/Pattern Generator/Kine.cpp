...
//	FK_RArm = new FwdKine(4); // 右手
	...
	if (selIK == LeftSupport)	// 平移一開始機器人座標 可調整GL 腳底板高度
	{
		RobotFixPoint[0] = 0;
		RobotFixPoint[1] = 80;
...
// from Matlab
	...
	// 以新量到的重量為主 必放在原本的後面 r_com則不用動 大致相等(軸的中點) 加電池	總重71208
	...
	mass_leg = 2*(mass_com[0] + mass_com[1] + mass_com[2]);	// 腳重 25800
	mass_upper = SumMass - mass_leg;	// 上身總重 45408
 ...
 // 多項式，用以計算swing腳軌跡
	...
	AngleErrLim = 0.001; // radian 0.001 = 0.057度
	...
	FlagArmAlone = 1;	// =0:使用COG補償(正常模式)		=1:手臂軌跡分離 不使用COG補償 WZ
	...
	Torq_I[0] = 0;
	Torq_I[1] = 0;
	for (int i = 0 ; i < 2*I_window ; ++i){
		Torq_I_add[i] = 0;	// integrate window
	}
	#endif

	#if OfflineTraj
		OfflineNum = 12400;	// 手動輸入離線的軌跡筆數 給馬達的encoder值 已乘過PNjoint
		KineFile.open("test9001.txt",ios::in);	// 讀回ENC變成COG 20130525 WZ
...
void Kine::FindFK(void)
{
	...
	******************************************************************/
	...
	// forward kinematics %calculate all the T matrixs
	FKLLeg->DHConstruct();
	FKRLeg->DHConstruct();
	FKLArm->DHConstruct();
	FKRArm->DHConstruct();
	...
	{
		// x axis -> joint 9 - joint 10 by foot position(different from ankle position)
		...
		for (int i = 0 ; i < LegDHLen; i++)
		{
			MatMulAtB(FixEndEffRMot,4,4,FKLLeg->Rn[i].data,4,4,temp44MatMul); //inverse mutiply Rotation(EndEff)   temp=R(end)^-1*Ri
			MatMulAB(LSwitchRMot,4,4,temp44MatMul,4,4,FKLLeg->Rn[i].data);// LSwitchRMot is the rotation matrix of the last time Tfinal=LSwitch*R(end)^-1*Ri
		}
		for (int i = 0 ; i < LegDHLen; i++)
		{
	...
	{			
		...
		if (OfflineTraj)
		{
			...
			RobotFixVector[2] = 0-(-704);//CrdAll->data[62];	// 調整GL高度	
		...
		else
		{
			...
			remLL[0] = CrdAll->data[18];	// rem 用在IK 因為IK的末端點其實是腳踝 不是腳板
			remLL[1] = CrdAll->data[19];
			remLL[2] = CrdAll->data[20];
			shiftLL[0] = CrdAll->data[21];	// shift 用在FK 因為FK的base是腳板 不是腳踝
...
void Kine::calc_float_Jacobian(double endeff[3],double * result )
{
	/************************************************************************
	 ...
	 NOTE: compute float jacobian
	     ...
	     special case:    J_floatBase_COG = [ eye(3) -cross2mat(Pb2COG)]; because COG has no rotation part
...
void Kine::calc_float_cogJacobian(double * result )
{
	/************************************************************************
	 input:     
	 output: double * result 
	 NOTE: compute float jacobian
	     special case:    J_floatBase_COG = [ eye(3) -cross2mat(Pb2COG)]; because COG has no rotation part
	 Examle: double cog[3]; double float_ja_cog[18]  calc_float_Jacobian(cog,float_ja_cog)
	...
	************************************************************************/
	...
	Mat_I_nxn(I3,3);
	MatMiuAB(COG,DHOrigin,P_b2j,3);
	cross2Mat(P_b2j,cross2mat);
	MatScalarMul(cross2mat,9,-1,cross2mat);

	Mat_A_addcol_B(I3,3,3,cross2mat,3,3,result);

}

void Kine::calc_sub_Jacobian(int SubIndex, double *result)
{
	/************************************************************************
	 input:    int SubIndex,double *result 
	 output: 
	 NOTE: compute fixed traditional jacobian  like Ja_LL and Ja_RL
    //SubIndex=1 Left Leg, 
			   2 Right Leg

	// Ja_LL = [Zx(end-axis[i].p)
	//			Z	]
	//Jf = [ J_floatBase_LL   Ja_LL        zeros(6,6);...
    //     J_floatBase_RL   zeros(6,6)   Ja_RL     ;...
    //     J_floatBase_COG  Ja_LL_COG    Ja_RL_COG ;...
	...
	************************************************************************/
	...
	int n =6; //column
	if(SubIndex==1)	//left leg sub jacobian	
	{	
		...
		ind_source = 0;
		ind_dest = 0;
		for (int i = 0 ; i <6 ; i++)//left腳每一軸到left腳endefector的距離
		{
		...
		ind_source = 0;
		ind_dest = 0;
		for (int i = 0 ; i < 6 ; i++) // 跑6次 1~6 column 1~3 row Z x r,  4~6 row  Z1,...,Z6
		{

			Cross2Vd(&ZAxisAll->data[ind_source],&EndEffDiff[ind_dest],temp_cross);
			result[0+i] = temp_cross[0];
			result[n+i] = temp_cross[1];
			result[2*n+i] = temp_cross[2];

			result[3*n+i] = ZAxisAll->data[ind_source];
			result[4*n+i] = ZAxisAll->data[ind_source+1];
			result[5*n+i] = ZAxisAll->data[ind_source+2];
	...
	else if(SubIndex==2)	//right leg sub jacobian	
	{	
		...
		ind_source = 39;
		ind_dest = 0;
		for (int i = 0 ; i <6 ; i++)//fixed(left)腳每一軸到swing(right)腳endefector的距離
		{
			EndEffDiff[ind_dest] =  EndEff[0] - CrdAll->data[ind_source] ;
			EndEffDiff[ind_dest+1] = EndEff[1] - CrdAll->data[ind_source+1] ;
			EndEffDiff[ind_dest+2] = EndEff[2] - CrdAll->data[ind_source+2] ;
...
void Kine::calc_sub_cogJacobian(int SubIndex, double *result)
{
	/************************************************************************
	 input:    int SubIndex,double *result 
	 output: 
	 NOTE: compute fixed traditional jacobian  like Ja_LL_COG and Ja_RL_COG
    //SubIndex=1 Left Leg, 
			   2 Right Leg

	// Ja_LL_COG = [Zx(cog-axis[i].p)]
    ...
    //     zeros(3,6)       -1.*Ja_LL(4:6,:) zeros(3,6)     ];  
			...
			last element of route is end point
	************************************************************************/
		int n =6; // column number

		if(SubIndex==1)//left leg
		{
		...
		result[0+5] = temp_cross[0]*temp_scale;
		result[n+5] = temp_cross[1]*temp_scale;
		result[2*n+5] = temp_cross[2]*temp_scale;

		// axis 5 LL ankle pitch
		Cross2Vd(&ZAxisAll->data[12],temp_r_ef,temp_cross);
		result[0+4] = temp_cross[0]*temp_scale;
		result[n+4] = temp_cross[1]*temp_scale;
		result[2*n+4] = temp_cross[2]*temp_scale;
		...
		result[0+3] = temp_cross[0]*temp_scale;
		result[n+3] = temp_cross[1]*temp_scale;
		result[2*n+3] = temp_cross[2]*temp_scale;
		...
		result[0+2] = temp_cross[0]*temp_scale;
		result[n+2] = temp_cross[1]*temp_scale;
		result[2*n+2] = temp_cross[2]*temp_scale;

		// axis 2 LL hip roll
		Cross2Vd(&ZAxisAll->data[3],temp_r_ef,temp_cross);
		result[0+1] = temp_cross[0]*temp_scale;
		result[n+1] = temp_cross[1]*temp_scale;
		result[2*n+1] = temp_cross[2]*temp_scale;

		// axis 1 LL hip yaw
		Cross2Vd(&ZAxisAll->data[0],temp_r_ef,temp_cross);
		result[0+0] = temp_cross[0]*temp_scale;
		result[n+0] = temp_cross[1]*temp_scale;
		result[2*n+0] = temp_cross[2]*temp_scale;
		}

		else if(SubIndex==2)//right leg
		{
...
void Kine::FloatJacobian(void)
{
	...
	******************************************************************/
	...
	double Ja_LL[36]; double Ja_RL[36];
	double float_ja_COG[18]; double Ja_LL_COG[18]; double Ja_RL_COG[18];
	...
	calc_sub_Jacobian(1, Ja_LL);
	calc_sub_Jacobian(2, Ja_RL);

	calc_float_cogJacobian(float_ja_COG);
	...
	double O3_12[36];
	Mat_O_mxn(O3_12,3,12);

	double tmp1[72];
	double tmp2[108];
	Mat_A_addcol_B(float_ja_LL,6,6,Ja_LL,6,6,tmp1);
	Mat_A_addcol_B(tmp1,6,12,O6,6,6,tmp2);

	double tmp3[72];
	double tmp4[108];
	Mat_A_addcol_B(float_ja_RL,6,6,O6,6,6,tmp3);
	Mat_A_addcol_B(tmp3,6,12,Ja_RL,6,6,tmp4);

	double tmp5[36];
	double tmp6[54];
	Mat_A_addcol_B(float_ja_COG,3,6,Ja_LL_COG,3,6,tmp5);
	Mat_A_addcol_B(tmp5,3,12,Ja_RL_COG,3,6,tmp6);

	double tmp7[18];
	double tmp8[54];
	Mat_A_addcol_B(O3,3,3,I3,3,3,tmp7);
	Mat_A_addcol_B(tmp7,3,6,O3_12,3,12,tmp8);
...
void Kine::ComputeJacobians(void)
{
	...
	******************************************************************/
	...
	if (selIK == LeftSupport) // support leg = left
	{
		...
		// 填寫角速度 Jacobian  fixed leg(left) to swing(right) leg
		...
		ind_source = 39;
		//int_dest 繼續加算
		for (int i = 6 ; i <12 ; i++)//swing(right)腳每一軸到swing腳endefector的距離
		{
			EndEffDiff[ind_dest] = EndEff[0] - CrdAll->data[ind_source];
			EndEffDiff[ind_dest+1] = EndEff[1] - CrdAll->data[ind_source+1];
			EndEffDiff[ind_dest+2] = EndEff[2] - CrdAll->data[ind_source+2];
...
//泓逸start111214
	...
	Index = 0;
	if (FlagArmAlone == 1)	// =0:使用COG補償(正常模式)		=1:手臂軌跡分離 不使用COG補償 COG計算的 if  要和位移分開
	{		
		if (gIthIK == 0){
			for (int i = 0 ; i < 8 ; ++i){
				pv_waist_Arm[i*3] = pv_stack[18+i*3] - DHOrigin[0];	// 這時的pv_stack已經是在各軸原點指向各軸重心的 向量(世界座標)
				pv_waist_Arm[i*3+1] = pv_stack[18+i*3+1] - DHOrigin[1];
				pv_waist_Arm[i*3+2] =  pv_stack[18+i*3+2] - DHOrigin[2];
			}
		}
		for (int i = 0 ; i < 8 ; ++i){
			pv_stack_Arm[i*3] = DHOrigin[0] + pv_waist_Arm[i*3];
			pv_stack_Arm[i*3+1] = DHOrigin[1] + pv_waist_Arm[i*3+1];
			pv_stack_Arm[i*3+2] = DHOrigin[2] + pv_waist_Arm[i*3+2];
		}
		for (int i = 0 ; i < 6 ; i++){	// 腳
		...
		for (int i = 6 ; i < 14 ; i++){	// 手
			COG[0] += pv_stack_Arm[Index-18]*mass_com[i];	// X方向不補
			COG[1] += pv_stack_Arm[Index-18+1]*mass_com[i];	// Y方向不補

			//COG[0] += pv_stack[Index]*mass_com[i];
			//COG[1] += pv_stack[Index+1]*mass_com[i];
			COG[2] += pv_stack_Arm[Index-18+2]*mass_com[i];	// 只有Z方向不補
			Index += 3;
		}
		for (int i = 14 ; i < 16 ; i++){ // 身體
			COG[0] += pv_stack[Index]*mass_com[i];
			COG[1] += pv_stack[Index+1]*mass_com[i];
			COG[2] += pv_stack[Index+2]*mass_com[i];
			Index += 3;
...
void Kine::GenSwingTraj(double v0, double a0, double x1_percentage,
	double y1, double v1, double x2, double y2, double v2, double a2, int Np, int KeepPosCount, double* resultXY, double* resultZ)
{
	/******************************************************************
	...
	******************************************************************/
	...
	while (1)
	{
		...
		if (NormXYZD(COGErr) >=  MaxCOGError)
		{
			...
			#if RunDynamics
			for (int i = 0 ; i < 3 ; ++i){
				COGErr[i] *= MaxCOGError/NormXYZD(COGErr);	// 限制最大速度
			}
			#else
			//system("pause");	
			#endif
...
void Kine::NumDiff(void)
{
	...
	******************************************************************/
	...
	else
	{
		...
		//fstream Fx;
		//Fx.open("TestAccelTotalCOM.txt",ios::app);		
		//for(int i = 0 ; i < 3 ; i++)
		//	Fx << AccelTotalCOM[i] <<"\t";
		//Fx << endl;
		//Fx.close();
		//Fx.open("TestVelTotalCOM.txt",ios::app);		
		//for(int i = 0 ; i < 3 ; i++)
		//	Fx << VelTotalCOM[i] <<"\t";
		//Fx << endl;
		//Fx.close();
...
////////////////////////////////////////注意! 以下在此是從swing腳的力規值往Fixed腳推算Joint的力//////////////////////////////////////////////////////////////
	if(selIK == 0 ||selIK == 2)	// Right leg lifiting & double support
	{
		...
		MatScalarMul(ForceCOM, 3, -1.0, ForceCOM);						// -ForceCOM333435 = -m333435 * ac333435
		MatAddAB(ForceCOM,Gravity,ForceJ+15+AxisJump,3);				// ForceJ333435 = -ma + mg
		MatAddAB(ForceJ+15+AxisJump,ForceDataKFR+(gIthIK+gInitCount)*6 ,ForceJ+15+AxisJump,3);// ForceSensor_forceR - ForceJ333435 + mg =  ForceCOM333435
	...
	else if(selIK == 1 )	// Left leg lifiting 
	{
		...
		MatScalarMul(ForceCOM, 3, -1.0, ForceCOM);			// -ForceCOM151617 = -m151617 * ac151617
		MatAddAB(ForceCOM, Gravity, ForceJ+15,3);			// -ma + mg
		MatAddAB(ForceJ+15, ForceDataKFL+(gIthIK+gInitCount)*6, ForceJ+15,3);	// ForceSensor_forceL - ForceJ151617 + m151617 * g =  ForceCOM151617
...
//{
	if(selIK==2||selIK == 0)// Right leg lifiting 
	{
		...
		// Swing Foot
			...
			Cross2Vd(ForceDataKFR+(gIthIK+gInitCount)*6, TempVector, FCrossR2);	// force Cross r

			// 計算該軸力矩
			MatAddAB(ForceDataKFR+(gIthIK+gInitCount)*6+3, FCrossR2, TempVector,3);
	...
	else if(selIK == 1 )// Left leg lifiting 
	{
		...
		// Swing Foot
			...
			MatAddAB(TempVector, SensorOffset, TempVector,3);
			Cross2Vd(ForceDataKFL+(gIthIK+gInitCount)*6, TempVector, FCrossR2);	// force Cross r
		...
		//Fx.close();
			...
			// 計算該軸力矩
			MatAddAB(ForceDataKFL+(gIthIK+gInitCount)*6+3, FCrossR2, TempVector,3);
...
void Kine::IMUSensorData(int LogCount,float *accex ,float *accey,float *accez, float *angvelx,float *angvely,float *angvelz, float *roll, float *pitch, float *yaw, float *rollF, float *pitchF, float *yawF, float *pos_x, float *pos_y, float *pos_z)
{
	//從ADAMS讀入sensor data
	*(accex+LogCount) = AdamsIMU[0];  //   單位mm/s2(from adams)  lateral 方向 
	*(accey+LogCount) = -AdamsIMU[2];//
	*(accez+LogCount) = -AdamsIMU[1];

	*(angvelx+LogCount) = AdamsIMU[3];   //roll  單位 degree/s       
	*(angvely+LogCount)=  AdamsIMU[5];//ok
	*(angvelz+LogCount) = -AdamsIMU[4];

	*(pitch+LogCount) = -1*AdamsIMU[9]+90;
	*(roll+LogCount) =  AdamsIMU[10];
	*(yaw+LogCount) =  AdamsIMU[11]-90;

	*(pos_x+LogCount) = -AdamsIMU[8]-1040;//OK 20140921
	*(pos_y+LogCount) = AdamsIMU[6]-256;//OK
	*(pos_z+LogCount) = AdamsIMU[7];//不確定要不要改

//void sensorfilter( float *datain , float  *dataout ) {
	...
	if (LogCount > 3)
	{
		pitchF[LogCount] =b1*pitch[LogCount] + b2 *pitch[LogCount-1] + b3 *pitch[LogCount-2] + b4*pitch[LogCount-3] - a2*pitchF[LogCount-1]- a3*pitchF[LogCount-2] -a4*pitchF[LogCount-3];
		rollF[LogCount] =b1*roll[LogCount] + b2 *roll[LogCount-1] + b3 *roll[LogCount-2] + b4*roll[LogCount-3] - a2*rollF[LogCount-1]- a3*rollF[LogCount-2] -a4*rollF[LogCount-3];
		yawF[LogCount] =b1*yaw[LogCount] + b2 *yaw[LogCount-1] + b3 *yaw[LogCount-2] + b4*yaw[LogCount-3] - a2*yawF[LogCount-1]- a3*yawF[LogCount-2] -a4*yawF[LogCount-3];
	}

	////以下測試用
		//fstream Fx;
		//
		//Fx.open("y:/accx.txt",ios::app);	
		///*for(int i = 0 ; i < 12 ; i++)	*/
		//Fx << *(accex+LogCount) << "\t";
		//Fx << endl;
		//Fx.close();
		////fstream Fx;
		//Fx.open("y:/accy.txt",ios::app);	
		///*for(int i = 0 ; i < 12 ; i++)	*/
		//Fx << *(accey+LogCount) << "\t";
		//Fx << endl;
		//Fx.close();
		////fstream Fx;
		//Fx.open("y:/accz.txt",ios::app);	
		///*for(int i = 0 ; i < 12 ; i++)	*/
		//Fx << *(accez+LogCount) << "\t";
		//Fx << endl;
		//Fx.close();
...
//從ADAMS讀入sensor data   //double *IMUpitch  double *IMUyaw 借來存將LFX,LFZ 在 motion control thread 存取的矩陣也改變 20140319

void Kine::IMUSensorData_hsuan(int FlagSim,int LogCount,float *IMUaccex ,float *IMUaccey,float *IMUaccez, float *IMUangvelx,float *IMUangvely,float *IMUangvelz ,
	double *LFX,double *LFZ ,float *IMUpitch ,double *waistthetadot){
		...
		// pitch roll yaw 
		// AdamsFS[12]  AdamsFS[13]  AdamsFS[14]


		if(FlagSim == ADAMSSimu){
	

		 *(IMUaccex+LogCount) =  AdamsFS[12];  //   單位 mm/s2 (from adams)   sagittal  方向 
		 		 	 
		 *(IMUaccey+LogCount) =  AdamsFS[14]; //
		 		 
		 *(IMUaccez+LogCount) = -AdamsFS[13]; //    單位 mm/s2 (from adams)   lateral  方向
		 

		  

		 *(IMUangvelx+LogCount) = AdamsFS[15];   // roll  單位 rad/s       
		 
		 *(IMUangvely+LogCount)=  AdamsFS[17];   // ok   
		 
		 *(IMUangvelz+LogCount) = -AdamsFS[16];

		 //借來存FX,FZ
		
		 *(LFX+LogCount) = -AdamsFS[0];  // 力規量到的值加負號  表示機器人與外界的交互力  GROUND REACTION FORCE (從後推是往前)       
		 *(LFZ+LogCount) = AdamsFS[2];	



		//*(IMUpitch+LogCount) = AdamsFS[19];
		//*(IMUroll+LogCount) =  AdamsFS[18];
		*(IMUpitch+LogCount) =  AdamsFS[18];
...
//void Kine::IMUData(int FlagSim, int FlagGo, int SensorCount, int LogCount, float *IMU_accelx, float *IMU_accely, float *IMU_accelz, 
//	float *IMU_omegax, float *IMU_omegay, float *IMU_omegaz, float *IMU_anglex,  float *IMU_angley,  float *IMU_anglez)
//{
//	/******************************************************************
//	input: void
//	output: void
...
//	******************************************************************/
//	int DataCount_IMU = SensorCount+LogCount;
//
...
//
//	if(FlagSim == ADAMSSimu){
//		IMU_accelx[DataCount_IMU] = AdamsIMU[0];
//		IMU_accely[DataCount_IMU] = AdamsIMU[1];
//		IMU_accelz[DataCount_IMU] = AdamsIMU[2];	
//		IMU_omegax[(DataCount_IMU)*6+3] = AdamsIMU[3];
//		IMU_omegay[(DataCount_IMU)*6+4] = AdamsIMU[4];
//		IMU_omegaz[(DataCount_IMU)*6+5] = AdamsIMU[5];
//
//		IMU_anglex[(DataCount_IMU)*6+3] = AdamsIMU[9];
//		IMU_angley[(DataCount_IMU)*6+4] = AdamsIMU[10];
//		IMU_anglez[(DataCount_IMU)*6+5] = AdamsIMU[11];
//
...
//		KineFile.close();
//
//		//KineFile.open("TestAdamsSimOriginal.txt",ios::in);	// 測試ADAMS 的KF 係數用
//		//for (int i = 0 ; i < 8215  ; ++i){
//		//	for (int j = 0 ; j < 12 ; ++j){
//		//		if ( j < 6 ){KineFile >> FS_DataL[i*6+j];} 
//		//		else{KineFile >> FS_DataR[i*6+j-6];}
//		//	}
//		//}
//		//KineFile.close();
//		//for (int i = 0 ; i < 12 ; ++i){	// ADAMS 多濾一點
...
void Kine::ForceSensorData(int FlagSim, int FlagGo, int FSCount, double *FS_DataL, double *FS_DataR)
{
	/******************************************************************
	input: void
	output: void
	...
	******************************************************************/
	...
	// Adams單位 F: N   T: N*mm
	if(FlagSim == ADAMSSimu){
		FS_DataL[FSCount*6] = AdamsFS[0];
		FS_DataL[FSCount*6+1] = -AdamsFS[1];
		FS_DataL[FSCount*6+2] = AdamsFS[2];	
		FS_DataL[FSCount*6+3] = AdamsFS[3]*0.001;
		FS_DataL[FSCount*6+4] = -AdamsFS[4]*0.001;	
		FS_DataL[FSCount*6+5] = AdamsFS[5]*0.001;	
		FS_DataR[FSCount*6] = AdamsFS[6];
		FS_DataR[FSCount*6+1] = -AdamsFS[7];
		FS_DataR[FSCount*6+2] = AdamsFS[8];
		FS_DataR[FSCount*6+3] = AdamsFS[9]*0.001;
		FS_DataR[FSCount*6+4] = -AdamsFS[10]*0.001;
		FS_DataR[FSCount*6+5] = AdamsFS[11]*0.001;

		for (int i = 0 ; i < 12 ; ++i)	// ADAMS 多濾一點
		{
			Q_KF[i] = 0.01;//Q_KF[i] = 0.01;	// Q為model的coverence 越小越相信
			R_KF[i] = 10;//R_KF[i] = 10;	// R為measuremant的coverence 越小越相信	
		}
	}

	// 在不同狀況下給力規資料
	// 輸入力規raw data txt檔單位一律為 F: N   T:N*m	
	if(FlagSim == CppSimu && FSCount ==0){
		KineFile.open("force_data_LLeg.txt",ios::in);
		for (int i = 0 ; i <SensorBufferSize  ; i++){
			KineFile >> FS_DataL[i];
		}
	...
	for(int i = 0 ; i < 6 ; i ++){
		if(FSCount == 0 && FlagGo == 1){
			x_est_last[i] = FS_DataL[i];// 尚未傳值前 第一筆資料直接當預測值
			x_est_last[i+6] = FS_DataR[i];
		}
		z_measured[i] = FS_DataL[FSCount*6+i];
		z_measured[i+6] = FS_DataR[FSCount*6+i];
	}
	...
	ForceDataKFL[FSCount*6] = x_est[0]*(1e+6);
	ForceDataKFL[FSCount*6+1] = x_est[1]*(1e+6);
	ForceDataKFL[FSCount*6+2] = x_est[2]*(1e+6);
	ForceDataKFL[FSCount*6+3] = x_est[3]*(1e+9);
	ForceDataKFL[FSCount*6+4] = x_est[4]*(1e+9);
	ForceDataKFL[FSCount*6+5] = x_est[5]*(1e+9);
	ForceDataKFR[FSCount*6] = x_est[6]*(1e+6);
	ForceDataKFR[FSCount*6+1] = x_est[7]*(1e+6);
	ForceDataKFR[FSCount*6+2] = x_est[8]*(1e+6);
	ForceDataKFR[FSCount*6+3] = x_est[9]*(1e+9);
	ForceDataKFR[FSCount*6+4] = x_est[10]*(1e+9);
	ForceDataKFR[FSCount*6+5] = x_est[11]*(1e+9);

	//ForceDataKFL[FSCount*6] = z_measured[0]*(1e+6);//test no filter
	//ForceDataKFL[FSCount*6+1] = z_measured[1]*(1e+6);
	//ForceDataKFL[FSCount*6+2] = z_measured[2]*(1e+6);
	//ForceDataKFL[FSCount*6+3] = z_measured[3]*(1e+9);
	//ForceDataKFL[FSCount*6+4] = z_measured[4]*(1e+9);
	//ForceDataKFL[FSCount*6+5] = z_measured[5]*(1e+9);
	//ForceDataKFR[FSCount*6] = z_measured[6]*(1e+6);
	//ForceDataKFR[FSCount*6+1] = z_measured[7]*(1e+6);
	//ForceDataKFR[FSCount*6+2] = z_measured[8]*(1e+6);
	//ForceDataKFR[FSCount*6+3] = z_measured[9]*(1e+9);
	//ForceDataKFR[FSCount*6+4] = z_measured[10]*(1e+9);
	//ForceDataKFR[FSCount*6+5] = z_measured[11]*(1e+9);
	...
	if(FlagGo==0){	// Go按下 CrdAll開始有值	
		// X = (x*Fz-z*Fx-My)/Fz
		// Y = (y*Fz-z*Fy+Mx)/Fz
		OrigCoPLx = (-ForceDataKFL[FSCount*6+4] - (60)*ForceDataKFL[FSCount*6] + (CrdAll->data[21]*ForceDataKFL[FSCount*6+2]) ) / ForceDataKFL[FSCount*6+2];
		OrigCoPLy = (ForceDataKFL[FSCount*6+3] - (60)*ForceDataKFL[FSCount*6+1] + (CrdAll->data[22]*ForceDataKFL[FSCount*6+2]) ) / ForceDataKFL[FSCount*6+2];
		OrigCoPRx = (-ForceDataKFR[FSCount*6+4] - (60)*ForceDataKFR[FSCount*6] + (CrdAll->data[60]*ForceDataKFR[FSCount*6+2]) ) / ForceDataKFR[FSCount*6+2];
		OrigCoPRy = (ForceDataKFR[FSCount*6+3] - (60)*ForceDataKFR[FSCount*6+1] + (CrdAll->data[61]*ForceDataKFR[FSCount*6+2]) ) / ForceDataKFR[FSCount*6+2];

		if(ForceDataKFL[FSCount*6+2] < 20*(1e+6)){	//左腳離地(< 20N) SSP ZMP不看左腳力規值 右腳為原點SUP
			CoPL[FSCount*2] = TarRotMSw[0] * OrigCoPLx + TarRotMSw[1] * OrigCoPLy;
			CoPL[FSCount*2+1] = TarRotMSw[3] * OrigCoPLx + TarRotMSw[4] * OrigCoPLy;
			CoPR[FSCount*2] = TarRotMFx[0] * OrigCoPRx + TarRotMFx[1] * OrigCoPRy;
			CoPR[FSCount*2+1] = TarRotMFx[3] * OrigCoPRx + TarRotMFx[4] * OrigCoPRy;
						
			FS_ZMP[FSCount*2] = CoPR[FSCount*2];
			FS_ZMP[FSCount*2+1] = CoPR[FSCount*2+1];
		}
		else if(ForceDataKFR[FSCount*6+2] < 20*(1e+6)){	//右腳離地(< 20N) SSP ZMP不看右腳力規值 左腳為原點SUP
			CoPL[FSCount*2] = TarRotMFx[0] * OrigCoPLx + TarRotMFx[1] * OrigCoPLy;
			CoPL[FSCount*2+1] = TarRotMFx[3] * OrigCoPLx + TarRotMFx[4] * OrigCoPLy;
			CoPR[FSCount*2] = TarRotMSw[0] * OrigCoPRx + TarRotMSw[1] * OrigCoPRy;
			CoPR[FSCount*2+1] = TarRotMSw[3] * OrigCoPRx + TarRotMSw[4] * OrigCoPRy;
						
			FS_ZMP[FSCount*2] = CoPL[FSCount*2];
			FS_ZMP[FSCount*2+1] = CoPL[FSCount*2+1];
		}
		else{	//DSP					
			...
			FS_ZMP[FSCount*2] = (ForceDataKFL[FSCount*6+2]*CoPL[FSCount*2] + ForceDataKFR[FSCount*6+2]*CoPR[FSCount*2]) / (ForceDataKFL[FSCount*6+2] + ForceDataKFR[FSCount*6+2]);
			FS_ZMP[FSCount*2+1] = (ForceDataKFL[FSCount*6+2]*CoPL[FSCount*2+1] + ForceDataKFR[FSCount*6+2]*CoPR[FSCount*2+1]) / (ForceDataKFL[FSCount*6+2] + ForceDataKFR[FSCount*6+2]);
		}
	}
	else{	// Go前 CrdAll無值 DSP (兩腳都在原點 所以Xs = 0)
		CoPL[0] = (-ForceDataKFL[FSCount*6+4] - (60)*ForceDataKFL[FSCount*6] ) / ForceDataKFL[FSCount*6+2];
		CoPL[1] = (ForceDataKFL[FSCount*6+3] - (60)*ForceDataKFL[FSCount*6+1] + (80*ForceDataKFL[FSCount*6+2]) ) / ForceDataKFL[FSCount*6+2];
		CoPR[0] = (-ForceDataKFR[FSCount*6+4] - (60)*ForceDataKFR[FSCount*6] ) / ForceDataKFR[FSCount*6+2];
		CoPR[1] = (ForceDataKFR[FSCount*6+3] - (60)*ForceDataKFR[FSCount*6+1] + ((-80)*ForceDataKFR[FSCount*6+2]) ) / ForceDataKFR[FSCount*6+2];

		FS_ZMP[0] = (ForceDataKFL[FSCount*6+2]*CoPL[0] + ForceDataKFR[FSCount*6+2]*CoPR[0]) / (ForceDataKFL[FSCount*6+2] + ForceDataKFR[FSCount*6+2]);
		FS_ZMP[1] = (ForceDataKFL[FSCount*6+2]*CoPL[1] + ForceDataKFR[FSCount*6+2]*CoPR[1]) / (ForceDataKFL[FSCount*6+2] + ForceDataKFR[FSCount*6+2]);			
	}
	// 以下測試用
	fstream Fx;
	Fx.open("FS_ZMP0.txt",ios::app);	
	Fx << FS_ZMP[FSCount*2] << "\t";
	Fx << endl;
	Fx.close();
	//fstream Fx;
	Fx.open("FS_ZMP1.txt",ios::app);	
	Fx << FS_ZMP[FSCount*2+1] << "\t";
	Fx << endl;
	Fx.close();
	//int test_stop=0;//test for stop
//		fstream Fx;
		Fx.open("TestX_est.txt",ios::app);	
		for(int i = 0 ; i < 12 ; i++)	
			Fx << x_est[i] << "\t";
		...
		Fx << ForceDataKFL[FSCount*6] << "\t"<< ForceDataKFL[FSCount*6+1] << "\t"<< ForceDataKFL[FSCount*6+2] << "\t";
		Fx << endl;
		Fx.close();

		Fx.open("TestFSensor_forcR.txt",ios::app);		
		Fx << ForceDataKFR[FSCount*6] << "\t"<< ForceDataKFR[FSCount*6+1] << "\t"<< ForceDataKFR[FSCount*6+2] << "\t";
		Fx << endl;
		Fx.close();

		Fx.open("TestFSensor_TorqL.txt",ios::app);		
		Fx << ForceDataKFL[FSCount*6+3] << "\t"<< ForceDataKFL[FSCount*6+4] << "\t"<< ForceDataKFL[FSCount*6+5] << "\t";
		Fx << endl;
		Fx.close();
	...
	******************************************************************/
	...
	if(FirstEncCOG==true){
		...
		if (selSupport[Enc_stepIndex] == LeftSupport){
			...
			RobotFixVector[1] = 0.5*(CrdAll->data[22]-CrdAll->data[61])-CrdAll->data[22];
			RobotFixVector[2] = 0-CrdAll->data[23];	// 這裡可以調整一開始GL腳底板高度
		}
		else if (selSupport[Enc_stepIndex] == RightSupport || selSupport[Enc_stepIndex] == DoubleSupport){
			RobotFixVector[0] = 0-CrdAll->data[60];
			RobotFixVector[1] = 0.5*(CrdAll->data[61]-CrdAll->data[22])-CrdAll->data[61];
			RobotFixVector[2] = 0-CrdAll->data[62];	// 這裡可以調整一開始GL腳底板高度
		}
		for (int i = 0 ; i < LegDHLen*2+ArmDHLen*2; i++){
			CrdAll->data[WriteIndex] += RobotFixVector[0];
			CrdAll->data[WriteIndex+1] += RobotFixVector[1];
		...
		Enc_shiftRL[0] = CrdAll->data[60];	// 右腳腳板
		Enc_shiftRL[1] = CrdAll->data[61];
		Enc_shiftRL[2] = CrdAll->data[62];
			
		Enc_shiftLL[0] = CrdAll->data[21];	// 左腳腳板 0430 WZ 一開始就記下左右腳位置 以利控制 應該不會影響ENC運行
		Enc_shiftLL[1] = CrdAll->data[22];
		Enc_shiftLL[2] = CrdAll->data[23];
	}
		Enc_foot[0] = CrdAll->data[21];
		Enc_foot[1] = CrdAll->data[22];
		Enc_foot[2] = CrdAll->data[23];
		Enc_foot[3] = CrdAll->data[60];
		Enc_foot[4] = CrdAll->data[61];
		Enc_foot[5] = CrdAll->data[62];
	...
	for (int i = 0 ; i < 3 ; i++){
		...
		GL_Enc_COG[i] = COG[i];	// GL_Enc_COG為了在GL可以畫圖 因為MainLoop沒有include LogEncCount
	}
	//////////////////////////////////////////////////////////////////////////
	//下一步要換腳
...
void Kine::Distri_Ratio(int StepNum, int FKCount, int IKCount, int HomePos)
{
	/******************************************************************
	input: 
	output: void
	...
	******************************************************************/
	int HomeCount;
	int end_step = 100;
	int Sig_Max = 50;	// 比重MAX
	double Sig_decay;// = 0.07;
	double Sig_raise;
	
	int Nshift;// = Nza*0.25;	
	double ratioL[360];	//180 + 180*0.25 = 225
	double ratioR[360];
	
	//////// TUNE
	//bool TuneRecordfile = false;
	//FILE *file;
	//char fileName[100];
	double Sig_a;	
	//////// TUNE
	...
	// 確定參數 還要再確認! 0515
		Sig_decay = 0.09;
		Sig_raise = 1- Sig_decay;
		Sig_a = 0.04;
		Nshift = Nza*1;
	//

	if (gFlagSimulation == CppSimu){
		Weight_cali = SumMass*GravityConst;	// 直接用C++重量 應該不太對 因為對應FS 是要依輸入的txt做總重
	}
	else if (gFlagSimulation == ADAMSSimu){
		Weight_cali = 680*(1e+6);	// 0430 由ADAMS模擬結果直接指定				
	}
	...
	GAF[2] = Weight_cali;
	// L leg	
	CotPoint[0] = 114;CotPoint[8] = 78;		CotPoint[1] = 114;CotPoint[9] = -62;
	CotPoint[3] = -96;CotPoint[11] = 78;	CotPoint[2] = -96;CotPoint[10] = -62;
	// R leg
	CotPoint[5] = 114;CotPoint[13] = 62;	CotPoint[4] = 114;CotPoint[12] = -78;
	CotPoint[6] = -96;CotPoint[14] = 62;	CotPoint[7] = -96;CotPoint[15] = -78;
				
	//// 比例
	RatioRe[0] = 1;	// 雖然蹲姿完全不用比例 但仍需要分配積分補償比重
	RatioRe[1] = 1;

	if (FlagStayMode == 1 || selSupport[2] == DoubleSupport || FlagStaySquatMode == 1){	// 站直不解IK  || 蹲姿給步數 || 蹲姿鍵盤暫停
		//HomeCount = 300;
		...
		// do nothing 		RatioRe[0] [1] 已指定為1 1 且不會再更動 再確認!!!!! 0528 WZ
	}
	else{	// 走路
		for (int j = 1 ; j < FKCount+Nza+Nab ; j++){	// 開始都是零 FK~IK[0~gNab]確定是零 140407
			RatioRe[j*2] = 1;	// 比例
			RatioRe[j*2+1] = 1;
		}
		...
		for (int i = 0 ; i < end_step ; i++){
			//// 分配DSP ratio
			if (i == 0){	// 第一步特別處理 selSupport[0] 原地重心右移 A不變 只改變權重
				if (selSupport[i+1] == LeftSupport){	// 下一步是左腳SUP
					SigmoidFunc(Nzb+Nshift, 1, 1, ratioL, 0.5*(Nzb+Nshift-1), Sig_a);
					SigmoidFunc(Nzb+Nshift, 1, Sig_Max, ratioR, Sig_raise*(Nzb+Nshift-1), Sig_a);
				}
				else if (selSupport[i+1] == RightSupport){	// 下一步是右腳SUP
					SigmoidFunc(Nzb+Nshift, 1, Sig_Max, ratioL, Sig_raise*(Nzb+Nshift-1), Sig_a);
					SigmoidFunc(Nzb+Nshift, 1, 1, ratioR, 0.5*(Nzb+Nshift-1), Sig_a);
				}
			}		
			else{
				if (selSupport[i+1] == LeftSupport){	// 下一步是左腳
					SigmoidFunc(Nzb+Nshift, Sig_Max, 1, ratioL, Sig_decay*(Nzb+Nshift-1), Sig_a);
					SigmoidFunc(Nzb+Nshift, 1, Sig_Max, ratioR, Sig_raise*(Nzb+Nshift-1), Sig_a);
				} 
				else if (selSupport[i+1] == RightSupport){	// 下一步是右腳
					SigmoidFunc(Nzb+Nshift, 1, Sig_Max, ratioL, Sig_raise*(Nzb+Nshift-1), Sig_a);
					SigmoidFunc(Nzb+Nshift, Sig_Max, 1, ratioR, Sig_decay*(Nzb+Nshift-1), Sig_a);
				}
				else if (selSupport[i+1] == DoubleSupport){	// 下一步是最後一步	注意contact point matrix A
					end_step = i+1;
			...
			for (int j = i*gStepSample+Nza+Nab ; j < (i+1)*gStepSample+Nza+Nab ; j++){	// j 不包含第一段FK的1000點							
				if (j >= end_step*gStepSample + Nshift){	// final step Nshift 到 final SSP
					RatioRe[(FKCount+j)*2] = 1;
					RatioRe[(FKCount+j)*2+1] = 1;
				} 
				else if(j < ((i+1)*gStepSample+Nshift)){	// DSP						
					RatioRe[(FKCount+j)*2] = ratioL[j-(i*gStepSample+Nza+Nab)];
					RatioRe[(FKCount+j)*2+1] = ratioR[j-(i*gStepSample+Nza+Nab)];							
				}
				else{	// SSP
					if (selSupport[i+1] == LeftSupport){	// 下一步是左腳SUP
						RatioRe[(FKCount+j)*2] = 1;
						RatioRe[(FKCount+j)*2+1] = Sig_Max*Sig_Max;//ratioR[Nzb+Nshift-1];
					} 
					else if (selSupport[i+1] == RightSupport){	// 下一步是右腳SUP
						RatioRe[(FKCount+j)*2] = Sig_Max*Sig_Max;//ratioL[Nzb+Nshift-1];
						RatioRe[(FKCount+j)*2+1] = 1;
					}
				}
			}
		...
		for (int i = FKCount+(end_step+1)*gStepSample-Nzb ; i < FKCount+IKCount+HomeCount; i++){	// 從最後一步的DSP 一路填到 end of scenario
			RatioRe[i*2] = 1;
			RatioRe[i*2+1] = 1;
		}
	}
...
void Kine::Distributor(double *ZMPxd, double *ZMPyd, int FKCount, int Tick, double Torq_gain)
{
	/******************************************************************
	input: 
	output: void
	...
	// StepNum = IK點數
	// FKCount = 開頭FK點數
	// swing腳離開地面在1000(FK)+180+480+180-45(非上下樓梯 一般情況下 NWait = Nza*0.25) 並不是在Nab才開始抬腳
	// Weighted Pseudo inverse x = inv(Q0) pinv( A(inv(Q0)) ) y
	// pinv( A(inv(Q0)) ) = A' inv(AA')
	// so x = inv(Q0) (A inv(Q0))' inv( (A inv(Q0))  (A inv(Q0))') y
	// 其中Q0 為權重對角矩陣
	// 20131011 WZ
	// 抬腳時機不對 與點數分配的函數無關
	******************************************************************/
	...
	double ErrorBound = 5*(1e+11);
	int Online = 0;	// 是否線上更改平衡點

	// 指定A[]
	if (Online == 1){	// 線上調整平衡點
		GAF[0] = GainX*( FS_ZMP[Tick*2] - ZMP[0] );	////////////////使用了未初始化的區域變數 'ZMP'
		GAF[1] = GainY*( FS_ZMP[Tick*2+1] - ZMP[1] );

		for (int k = 0 ; k < 4 ; k++){
			A[k] = CotPoint[k]+Enc_foot[0];			A[4+k] = CotPoint[k+4]+Enc_foot[3];
			A[8+k] = CotPoint[k+8]+Enc_foot[1];		A[12+k] = CotPoint[k+12]+Enc_foot[4];	// 注意ENC有沒有輸入!
			A[16+k] = 1;							A[20+k] = 1;
		}
	}
	else{
		for (int k = 0 ; k < 4 ; k++){
			A[k] = CotPoint[k]+CrdAll->data[21];			A[4+k] = CotPoint[k+4]+CrdAll->data[60];
			A[8+k] = CotPoint[k+8]+CrdAll->data[22];		A[12+k] = CotPoint[k+12]+CrdAll->data[61];
			A[16+k] = 1;									A[20+k] = 1;
		}
	}

	//  開始分力 LogCount == 0 
	if (FlagStayMode == 1 || selSupport[2] == DoubleSupport || FlagStaySquatMode == 1 || Tick < FKCount+Nza+Nab)
	{	
		// 站直不解IK  || 蹲姿給步數 || 蹲姿鍵盤暫停 || 走路FK~IK[0~gNab] A[] ZMP 不會變 會變的是GAF[]
		ZMP[0] = 0;
		ZMP[1] = 0;
		ZMP[2] = 1;

		// 無權重的pseudo inverse (權重ratio為1 1)	///////////////////////////////還要加入GAF[0]GAF[1]
	...
	{
		ZMP[0] = ZMPxd[Tick-FKCount];
		ZMP[1] = ZMPyd[Tick-FKCount];
		ZMP[2] = 1;
		...
		for (int m = 0 ; m < 8 ; m++){	// Q0對角矩陣 對角值為forceratio開根號
			for (int n = 0 ; n < 8 ; n++){
				if(m == n){
					if (m < 4){	// L
						Q0[8*m+n] = RatioRe[Tick*2];}
					else{	// R
						Q0[8*m+n] = RatioRe[Tick*2+1];}
				}
				else{
					Q0[8*m+n] = 0;
				}
		...
		//x = inv(Q0) (A inv(Q0))' inv( (A inv(Q0))  (A inv(Q0))') y
		InvSqMat(Q0,8);	// inv(Q0)
		MatMulAB(A,3,8,Q0,8,8,A_w);	// A inv(Q0)

		MatScalarMul(ZMP,3,GAF[2],force_temp);	// y

		MatMulABt(A_w,3,8,A_w,3,8,pinv_temp);	// (A inv(Q0))  (A inv(Q0))'
		InvSqMat(pinv_temp,3);	// inv( (A inv(Q0))  (A inv(Q0))')
		MatMulAtB(A_w,3,8,pinv_temp,3,3,pinv_A_w);	// (A inv(Q0))' inv( (A inv(Q0))  (A inv(Q0))')

		MatMulAB(Q0,8,8,pinv_A_w,8,3,temp);	// inv(Q0) (A inv(Q0))' inv( (A inv(Q0))  (A inv(Q0))')

		MatMulAB(temp,8,3,force_temp,3,1,F_dis);	// inv(Q0) (A inv(Q0))' inv( (A inv(Q0))  (A inv(Q0))') y	
	}

	////蹲著也要補I 先做 在歸零Tick
	if (Tick > FKCount)	// 力矩積分補償
	{		
		for (int i = 0 ; i < 2 ; ++i){///////////////////////////////////////////////是否要改成while?
			Torq_err[i] = (FS_ZMP[Tick*2+i] - ZMP[i])*Weight_cali;
			for (int j = 0; j < (I_window-1) ; ++j){//////////////////////////////////////是否要改成while?
				Torq_I_add[2*(j+1)+i] = Torq_I_add[2*j+i];	// integrate window
			}

			Torq_I_add[i] = Torq_err[i];	// 最新一筆輸入到0 1

			for (int j = 0 ; j < I_window ; ++j){
				Torq_I[i] += Torq_I_add[2*j+i];	// sum
			}
			if (Torq_I[i] > ErrorBound){
				Torq_I[i] = ErrorBound;
			}
			else if (Torq_I[i] < -1*ErrorBound){
				Torq_I[i] = -1*ErrorBound;
			}
		}
	}


	if (FlagStayMode == 1 || selSupport[2] == DoubleSupport || FlagStaySquatMode == 1){	
		Tick = 0;	// 站直不解IK  || 蹲姿給步數 || 蹲姿鍵盤暫停 存到每段的第一筆
	}
	////只有走路才補I
	//else if (Tick > FKCount)	// 力矩積分補償
	//{		
	//	for (int i = 0 ; i < 2 ; ++i){
	//		Torq_err[i] = (ZMP[i] - FS_ZMP[Tick*2+i])*Weight_cali;
	//		Torq_I[i] += Torq_err[i];
	//		if (Torq_I[i] > ErrorBound){
	//			Torq_I[i] = ErrorBound;
	//		}
	...
	for (int Re = 0 ; Re < 8 ; ++Re){
		F_disRe[Tick*8+Re] =  F_dis[Re];
	}
	F_total[Tick*2] = F_dis[0]+F_dis[1]+F_dis[2]+F_dis[3];	// L Force
	F_total[Tick*2+1] = F_dis[4]+F_dis[5]+F_dis[6]+F_dis[7];	// R Force

	// 注意ratio分配 因為ratio是1~2500 故support腳的比例應是 2500/2500
	//Ratio_total = RatioRe[Tick*2] + RatioRe[Tick*2+1];
	//Torq_del[0] = RatioRe[Tick*2+1] * Torq_gain*Torq_I[0] / Ratio_total;	// L Pitch
	//Torq_del[1] = RatioRe[Tick*2+1] * Torq_gain*Torq_I[1] / Ratio_total;	// L Roll
	//Torq_del[2] = RatioRe[Tick*2] * Torq_gain*Torq_I[0] / Ratio_total;	// R Pitch
	//Torq_del[3] = RatioRe[Tick*2] * Torq_gain*Torq_I[1] / Ratio_total;	// R Roll

	Torq_del[0] = Torq_gain*Torq_I[0] / RatioRe[Tick*2];	// L Pitch
	Torq_del[1] = Torq_gain*Torq_I[1] / RatioRe[Tick*2];	// L Roll
	Torq_del[2] = Torq_gain*Torq_I[0] / RatioRe[Tick*2+1];	// R Pitch
	Torq_del[3] = Torq_gain*Torq_I[1] / RatioRe[Tick*2+1];	// R Roll

	T_total[Tick*4] = -1*( (F_dis[0]+F_dis[1])*CotPoint[0] + (F_dis[2]+F_dis[3])*CotPoint[2] ) + Torq_del[0];	// L Pitch
	T_total[Tick*4+1] = ( (F_dis[0]+F_dis[3])*CotPoint[8] + (F_dis[1]+F_dis[2])*CotPoint[9] ) + Torq_del[1];	// L Roll
	T_total[Tick*4+2] = -1*( (F_dis[4]+F_dis[5])*CotPoint[0] + (F_dis[6]+F_dis[7])*CotPoint[2] ) + Torq_del[2];	// R Pitchi
	T_total[Tick*4+3] = -1*( (F_dis[5]+F_dis[6])*CotPoint[12] + (F_dis[4]+F_dis[7])*CotPoint[13] ) + Torq_del[3];// R Roll

	AKHS_I[Tick*4] = Torq_del[0];	// L Pitch
	AKHS_I[Tick*4+1] = Torq_del[1];	// L Roll
	AKHS_I[Tick*4+2] = Torq_del[2];	// R Pitch
	AKHS_I[Tick*4+3] = Torq_del[3];	// R Roll

	// Torque方向為受力方向 pitch 一律加負號 為了配合CotPoint roll 右腳加負號????? 再確認

	//if (TuneRecordfile == true)
	//{
	//	sprintf(fileName,"D:/disTune/%d/TestF_dis_%.2f_%.2f.txt",Nshift+Nzb,Sig_decay,Sig_a);
	//	file = fopen(fileName,"w");
	//	for(int i = 0 ; i < FKCount+IKCount+HomeCount ; i++){
	//		fprintf(file, "%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\n", F_disRe[i*8], F_disRe[i*8+1], F_disRe[i*8+2], F_disRe[i*8+3], F_disRe[i*8+4], F_disRe[i*8+5], F_disRe[i*8+6], F_disRe[i*8+7]);
	//	}
	//	fclose(file);

	//	sprintf(fileName,"D:/disTune/%d/TestF_total_%.2f_%.2f.txt",Nshift+Nzb,Sig_decay,Sig_a);
	//	file = fopen(fileName,"w");
	//	for(int i = 0 ; i < FKCount+IKCount+HomeCount ; i++){
	//		fprintf(file, "%.5e\t%.5e\n", F_total[i*2], F_total[i*2+1]);
	//	}
	//	fclose(file);

	//	sprintf(fileName,"D:/disTune/%d/TestT_total_%.2f_%.2f.txt",Nshift+Nzb,Sig_decay,Sig_a);
	//	file = fopen(fileName,"w");
	//	for(int i = 0 ; i < FKCount+IKCount+HomeCount ; i++){
	//		fprintf(file, "%.5e\t%.5e\t%.5e\t%.5e\n", T_total[i*4], T_total[i*4+1], T_total[i*4+2], T_total[i*4+3]);
	//	}	
	//	fclose(file);
	//}
}

void Kine::SigmoidFunc(double x, double y1, double y2, double* result, double c, double a)
{
	/******************************************************************
	input: x1, x2, num, y1, y2
	output: * result

	Note:	
	// x橫向座標 num 為橫向內差點數 y1 y2 為值
	// From MATLAB 
	...
	******************************************************************/
	...
	for (int i = 0 ; i < x ; ++i)
	{
		result[i] = ( (y2-y1) / ( 1+ exp(-a*(i-c)) ) ) + y1;

		if (i == (x-1))	// 最後一筆時 記住平移的量
		{
			if ( y1 == 1 )	// 開頭是1
			{
				shiftto1 = result[0];
	...
	for (int j = 0 ; j < x ; ++j)
	{
		result[j] = result[j] - shiftto1 + 1;	// 平移到1的位置
	}
}