...
public:
	...
	void ForceSensorData(int FlagSim, int FlagGo, int FSCount, double *FS_DataL, double *FS_DataR);
	// 線上收力規值 濾波 或是離線收txt
	void FindEncCOG(double* Enc_theta, double* Enc_COG);
	// 利用Encoder Feedback透過FindFK FindCOG 算出COG
	void Distri_Ratio(int StepNum, int FKCount, int IKCount, int HomePos);
	// 決定每段劇情兩腳 Weighted Pseudo inverse
	void Distributor(double *ZMPxd, double *ZMPyd, int FKCount, int Tick, double Torq_gain);
	// 分配desired force to preset contact point
	void SigmoidFunc(double x, double y1, double y2, double* result, double c, double a);
	// 利用SigmoidFunction 做DSP重量分配

	void IMUSensorData(int LogCount,float *accex ,float *accey,float *accez, float *angvelx,float *angvely,float *angvelz, float *roll,float *pitch, float *yaw, float *rollF, float *pitchF, float *yawF,float *pos_x, float *pos_y, float *pos_z);//20140921 DORA change
	//void IMUData(int FlagSim, int FlagGo, int SensorCount, int LogCount, float *IMU_accelx, float *IMU_accely, float *IMU_accelz, 
	...
	double pv_stack_Arm[4*3*2];	// 只解下半身IK FindCOG用來記下各軸手臂COG 世界位置的矩陣
	double pv_waist_Arm[4*3*2];	// 只解下半身IK FindCOG用來記下腰到手臂COG 世界向量的矩陣
	double pv_waist[3];	// 只解下半身IK FindCOG用來記下腰進IK前的位置的矩陣
	...
	double remLL[3];	// 記住換腳時左腳的位置(Ankle) rem 用在IK 因為IK的末端點其實是腳踝 不是腳板
	double remRL[3];	// 記住換腳時右腳的位置(Ankle)
	double shiftLL[3];	// 計算腳固定在空間中的向量(腳底投影點 shift 用在FK 因為FK的base是腳板 不是腳踝
	double shiftRL[3];	// 計算腳固定在空間中的向量(腳底投影點)
	...
	double dthLeg[12];	// 解下半身IK 用
	double dxLeg[12];
	double tempJLeg[12*12];
	//泓逸end111227
	...
	bool FirstFKFound; // 第一次IK取完的旗標 一開始是false 做完第一次後變成true
	...
	int FlagArmAlone;	// 手臂軌跡分離 不使用COG補償
	...
	double AdamsIMU[12];	// 存Adams模擬IMU的值
	...
	// 利用ENC FK FindCOG得到的COG位置 以下為使用變數
		...
		double Enc_foot[6];	// 存ENC當下兩腳 腳踝投影點世界座標
		bool FirstEncCOG;
		int Enc_stepIndex;
		// Wei-Zh 130908
	// Ankel Strategy
		double F_total[2*20000];	// 假設非蹲姿IK 最多20000點
		double T_total[4*20000];
		double F_disRe[8*20000];
		double RatioRe[2*20000];
		double AKHS_I[4*20000];
		int SimuFail;
		int mass_leg;
		int mass_upper;
		double Weight_cali;	// Cali總重 unit: N(1e-6)
	// Distributor
		double CotPoint[16];
		double GAF[3];
