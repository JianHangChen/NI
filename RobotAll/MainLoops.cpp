...
void gOpenPort()
{
	...
	******************************************************************/
	...
	if(	 gStartSensing == true)
	{
		...
		gpPortLA->open("\\\\.\\COM16",NORMAL_RS232);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
		gpPortLA->_set_baudrate(115200);
		gpPortRA->open("\\\\.\\COM17",NORMAL_RS232);
		gpPortRA->_set_baudrate(115200);
//======================================================================// 至峻 20130410
		gpPortRH->open("\\\\.\\COM8",NORMAL_RS232);					
		gpPortRH->_set_baudrate(115200);
		gpPortLH->open("\\\\.\\COM19",NORMAL_RS232);					
...
void gLoadfileHand() // 至峻 20130410
{
	...
	char fileR[] = "Right_Hand_New.txt";
	char fileL[] = "Left_Hand_New.txt";
...
void gRenderSceneThread(void)
{
	...
	******************************************************************/
	...
	//_______________________________________Added by Slongz_______________________________________
				...
				glPushMatrix();
				glTranslatef((float)gKineAll.COG[1],(float)gKineAll.COG[2],(float)gKineAll.COG[0]);
				//glTranslatef((float)gKineAll.GL_Enc_COG[1],(float)gKineAll.GL_Enc_COG[2],(float)gKineAll.GL_Enc_COG[0]);	// 畫ENC_COG
				glColor3f(0.0f,1.0f,0.0f);
				glutSolidSphere(30.0f,20,20);
				glPopMatrix();
				#endif
				...
				//glPopMatrix();
				  ...
				  if(gFlagReadForceSensor==0)
				  {
						...
						//GLCOG[tempindex][0]=gKineAll.GL_Enc_COG[1];
						//GLCOG[tempindex][1]=gKineAll.GL_Enc_COG[2];
						//GLCOG[tempindex][2]=gKineAll.GL_Enc_COG[0];
						...
						if(gIthIK>0)
						{
							GLZMP[tempindex][0]=gLQs.XState[gIthIK].data[2]+gKineAll.initCOG[1];
							GLZMP[tempindex][1]=gInpZMPHeight[gIthIK];
							GLZMP[tempindex][2]=gLQs.YState[gIthIK].data[2]+gKineAll.initCOG[0];
						}
						else
						{
							GLZMP[tempindex][0]=gLQs.XState[0].data[2]+gKineAll.initCOG[1];
							GLZMP[tempindex][1]=gInpZMPHeight[0];
							GLZMP[tempindex][2]=gLQs.YState[0].data[2]+gKineAll.initCOG[0];
						}
						for(int i=0;i<tempindex;i++)
						{
						glPushMatrix();
...
//fstream aces;
	...
	switch(switchnumb)
	{
		...
		case 2://writingfile test (offlin ver.) 存每軸軌跡
		...
		case 3://formal communication
			while(1)
			{	
				tempfile.open("F:/Documents/Desktop/ADAMSmodel_hit_mech0819/test.txt",ios::in);
				for(int g=0;g<datasize+1;g++)
				{
					if(g < 12)
					tempfile>>gKineAll.AdamsFS[g];
					else if(g < 24)
					tempfile>>gKineAll.AdamsIMU[g-12];
					else
					tempfile>>M2C_ID;
				}
				...
				if(M2C_ID==127)
				{
					...
					tempfile.open("F:/Documents/Desktop/ADAMSmodel_hit_mech0819/test.txt",ios::out| ios::trunc);
					for(int i=0;i<datasize;i++)
					{
						tempfile<<data[dataindexstart+i]<<"\t";
					}
		...
		case 5://formal communicationz
			...
			while(1)
			{	
				tempfile.open("F:\Documents\Desktop\mattoc",ios::in);
				for(int g=0;g<datasize+1;g++)
				{
					if(g==0)
						tempfile>>tempZMP[0];
				...
				if(M2C_ID==127)
				{
					...
					tempfile.open("Y:/test.txt",ios::out| ios::trunc);
					for(int i=0;i<datasize;i++)
					{
						tempfile<<data[i]<<"\t";
					}
...
void C2MLoadTraj(int datanumb, double data[], int dataindexstart, fstream &tempfile)
{
	...
	{ 
		...
		case 12:  // Biped Traj
			for (int h = 0 ; h < 6 ; h++)
			{
				data[h]=gKineAll.FKLLeg->theta[h+1];	//弧度
		...
		case 34:  // Biped Traj & Footpad Location (projection of ankle posi. on the ground)
			for (int h = 0 ; h < 6 ; h++)
			{
				...
				data[12]=gKineAll.FKLArm->theta[1];	// 腰
		...
		case 35:  // Biped Traj & Footpad Location (projection of ankle posi. on the ground) + Rotation
			for (int h = 0 ; h < 6 ; h++)
			{
				if(h==4)
				{
